...
module Embarista
  module Helpers
    def precompile_handlebars(*args, &block)
      filter(PrecompileHandlebarsFilter, *args, &block)
    end

    def rewrite_minispade_requires(*args, &block)
      filter(RewriteMinispadeRequiresFilter, *args, &block)
    end

    def erb(*args, &block)
      filter(ErbFilter, *args, &block)
    end

    def strip_ember_asserts(*args, &block)
      filter(StripEmberAssertsFilter, *args, &block)
    end
